    ...
    internal class BankAccountRepository : RepositoryBase<BankAccount>, IBankAccountRepository
    {
        ...
        public async Task DeleteByIdentifier(string identifier)
        {
            var bankAccount = await _context.BankAccounts!
                .Where(x => x.Identifier == identifier).FirstOrDefaultAsync();

            if (bankAccount == null)
            {
                throw new Exception($"No se pudo eliminar la cuenta bancaria con el id {identifier}");
            }
        ...
        public async Task DepositBankAccountByIdentifier(BankAccount bankAccount)
        {
            ...
            if (account == null)
            {
                throw new Exception($"No se pudo actualizar en mysql la cuenta bancaria con el identifier {bankAccount.Identifier}");
            }
        ...
        public async Task WithdrawnBankAccountByIdentifier(BankAccount bankAccount)
        {
            ...
            {
                throw new Exception($"El balance de la cuenta es menor que el dinero que desea retirar para {bankAccount.Identifier}");
            }
